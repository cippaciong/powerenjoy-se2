...
\newcommand{\mockup}[2]{
	...
	\begin{figure}[H]
		...
		\includegraphics[width=\textwidth,height=0.93\textheight,keepaspectratio]{#1.png}
...
\mockup{reservation}{The page where the user can insert research details.}
\mockup{research}{Here is the page where the list of the available cars is shown.}
\mockup{map}{The page where the car's location is shown on the map.}

\subsection{Car screen}
\refstepcounter{MockupCounter}

\rotatebox{90}{
	\begin{minipage}[H]{0.97\textheight}
		
		
		\begin{figure}[H]
			\centering
			\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{screen.png}
			\caption*{Mockup \arabic{MockupCounter}: The interface of the car screen.}
			\label{mockup:screen}
		\end{figure}
